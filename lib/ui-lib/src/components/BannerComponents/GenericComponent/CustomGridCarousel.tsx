import { BANNER_ITEM_FRONTEND, CarouselGridConfig, ScreenSize } from "@/utils";
  ...
  return (
    <div className="w-full mx-auto relative">
      ...
      {showDots && scrollSnaps?.length > 1 && (
        <div className="flex justify-center absolute bottom-2 items-center w-full">
          <div className="bg-black/10 rounded-full flex justify-center items-center py-[3px] px-6 ">
            {scrollSnaps.map((_, index) => (
              <button
                key={index}
                className={`h-1 rounded-full mx-1 my-2 p-0 min-w-0 ${
                  index === selectedIndex ? "w-6 bg-white/90" : "w-1 bg-white/50"
                }`}
