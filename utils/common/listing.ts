...
export const fieldsToListingPage = ({
  ...
  const filtersString = arrayObjectToQueryString(filters);
  const tagsString = FilterTagsParse(tags || []);
  const finalString = allFilters.join("&");
  const quickFiltersString = [quickFiltersParse(quick_filter_tags || [])].join("&");
  const redirect = "/products?" + "banner_id=" + pk + "&namex=" + name;
  if (pk && pk !== -1) {
    // if (filtersString) {
    //   redirect += "&" + filtersString;
    // }
    // if (quickFiltersString) {
    //   redirect += "&" + quickFiltersString;
    // }
    // if (tagsString) {
    //   redirect += "&" + tagsString;
    // }
